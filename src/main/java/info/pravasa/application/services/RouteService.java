...
public class RouteService {
    ...
    public void save(RouteDto bean) {
        webClient.post().uri("http://localhost:8001/bus-service/route/save")
                .bodyValue(bean)
    ...
    public List<RouteDto> findAll(Long companyId) {
        if(Objects.isNull(companyId)){
            return Collections.emptyList();
        }
        return Objects.requireNonNull(webClient.post().uri("http://localhost:8001/bus-service/route/findAll")
                .bodyValue(companyId)
                .header("Authorization", VaadinSession.getCurrent().getAttribute("token").toString())
                .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
    ...
    public void saveJson(String json) {
        webClient.post()
                .uri("http://localhost:8001/bus-service/route/saveAll")
